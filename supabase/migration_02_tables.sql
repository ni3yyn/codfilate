-- ============================================
-- PART 2: TABLES, COLUMNS, RLS, TRIGGERS
-- Run AFTER migration_01_enums.sql is committed
-- Safe to re-run (IF NOT EXISTS / ON CONFLICT / OR REPLACE)
-- ============================================
...
-- ============================================
-- 5. SETTLEMENTS TABLE (Profit Distribution)
-- ============================================
...
-- ============================================
-- 6. WALLETS TABLE (balance per user)
-- ============================================
  ...
  IF NEW.status = 'delivered' AND (OLD.status IS NULL OR OLD.status != 'delivered') THEN
    ...
    IF v_order.affiliate_id IS NOT NULL THEN
      ...
      IF v_affiliate_user_id IS NOT NULL AND v_marketer_profit > 0 THEN
        INSERT INTO wallets (user_id, balance, total_earned) VALUES (v_affiliate_user_id, v_marketer_profit, v_marketer_profit)
        ON CONFLICT (user_id) DO UPDATE SET
          balance = wallets.balance + v_marketer_profit,
          total_earned = wallets.total_earned + v_marketer_profit,
          updated_at = now();
    ...
    IF v_order.regional_manager_id IS NOT NULL THEN
      ...
      IF v_manager_user_id IS NOT NULL THEN
        INSERT INTO wallets (user_id, balance, total_earned) VALUES (v_manager_user_id, 150, 150)
        ON CONFLICT (user_id) DO UPDATE SET
          balance = wallets.balance + 150,
          total_earned = wallets.total_earned + 150,
          updated_at = now();
      END IF;
    END IF;
    ...
    IF NEW.delivery_company_id IS NOT NULL THEN
      ...
      IF v_delivery_user_id IS NOT NULL AND v_delivery_fee > 0 THEN
        INSERT INTO wallets (user_id, balance, total_earned) VALUES (v_delivery_user_id, v_delivery_fee, v_delivery_fee)
        ON CONFLICT (user_id) DO UPDATE SET
          balance = wallets.balance + v_delivery_fee,
          total_earned = wallets.total_earned + v_delivery_fee,
          updated_at = now();
      END IF;
    END IF;
