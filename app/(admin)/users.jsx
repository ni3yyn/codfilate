...
export default function AdminUsers() {
  ...
  const renderUser = ({ item }) => {
    ...
    return (
      ...
      >
        <Card style={styles.userCard} accentColor={roleColor} accentPosition="left">
          <View style={styles.userRow}>
            ...
            <View style={styles.userInfo}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={[styles.userDate, { color: theme.colors.textTertiary }]}>انضم {formatDate(item.created_at)}</Text>
              </View>
            </View>
          ...
          {isExpanded && (
            <View style={[styles.expandedContent, { borderTopColor: theme.colors.divider }]}>
              <View style={[styles.detailsBox, { backgroundColor: theme.colors.shimmer }]}>
                ...
                {item.phone && (
                  <View style={styles.detailRow}>
                      <Ionicons name="call-outline" size={12} color={theme.colors.textSecondary} style={{ marginEnd: 4 }} />
                ...
                {item.store_id && (
                  <View style={styles.detailRow}>
                      <Ionicons name="storefront-outline" size={12} color={theme.colors.textSecondary} style={{ marginEnd: 4 }} />
                      <Text style={[styles.detailLabel, { color: theme.colors.textSecondary }]}>المتجر:</Text>
                    <Text
                      style={[styles.detailValue, { color: theme.colors.text }]}
                      numberOfLines={1}
                    >
                ...
                <View style={{ marginTop: 12, gap: 8 }}>
                  <Text style={[styles.detailLabel, { color: theme.colors.textSecondary }]}>تغيير الدور</Text>
                  ...
                  {item.role === 'regional_manager' && (
                    <Button
                      ...
                      variant="secondary"
                      onPress={() => router.push({ pathname: '/(admin)/assign-wilaya', params: { profileId: item.id } })}
                    />
                  )}
                </View>
              </View>
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]} edges={['bottom']}>
      <UniversalHeader 
        title="المستخدمين" 
        rightAction={<Badge label={`${users.length} المجموع`} variant="primary" />}
      />
          ...
          <TouchableOpacity
            ...
            style={[
              styles.filterBtn,
              {
                ...
                borderColor: roleFilter === f.key ? theme.primary : 'transparent',
                borderWidth: 1,
              },
            ]}
          >
            <Text
              style={[
                styles.filterText,
                { color: roleFilter === f.key ? '#FFFFFF' : theme.colors.textSecondary },
              ]}
            >
...
const styles = StyleSheet.create({
  ...
  detailValue: { ...typography.caption, fontFamily: 'Tajawal_500Medium', flex: 1.5, textAlign: 'left' },
});
