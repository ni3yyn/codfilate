...
export default function AdminStores() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [{ data: orderData }, { data: productData }, { data: affiliateData }] = await Promise.all([
  ...
  const handleToggleStatus = (store) => {
    ...
    showConfirm({
      ...
      onConfirm: async () => {
        ...
        if (!result.success) showAlert({ title: 'خطأ', message: result.error, type: 'error' });
        else showAlert({ title: 'تم', message: `تم ${newStatus ? 'تفعيل' : 'تعطيل'} المتجر بنجاح.`, type: 'success' });
      },
    });
  };
  ...
  const SummaryHeader = () => (
    <View style={styles.summaryContainer}>
      <Card style={styles.summaryCard}>
        <Text style={[styles.summaryLabel, { color: theme.colors.textSecondary }]}>إجمالي المتاجر</Text>
        <Text style={[styles.summaryValue, { color: theme.colors.text }]}>{platformStats.total}</Text>
      </Card>
      <Card style={styles.summaryCard}>
        <Text style={[styles.summaryLabel, { color: theme.colors.textSecondary }]}>المتاجر النشطة</Text>
        <Text style={[styles.summaryValue, { color: theme.colors.success }]}>{platformStats.active}</Text>
      </Card>
  ...
  const renderStore = ({ item }) => {
    ...
    const cardWidth = isDesktop ? (Math.min(width, 1400) - spacing.xl * 2) / 4 - spacing.md : isTablet ? (width - spacing.xl) / 2 - spacing.md : width - spacing.md * 2;
      ...
      <View style={{ width: cardWidth, margin: spacing.xs }}>
        ...
        >
          <View style={styles.cardHeader}>
            ...
            <View style={[styles.statusDot, { backgroundColor: item.is_active ? theme.colors.success : theme.colors.error }]} />
          </View>

          {/* Quick Stats Grid */}
          <View style={[styles.statsGrid, { backgroundColor: theme.colors.shimmer }]}>
            <View style={styles.statBox}>
              <Text style={[styles.statNum, { color: theme.colors.text }]}>{stats.orders}</Text>
              <Text style={[styles.statName, { color: theme.colors.textTertiary }]}>طلب</Text>
            </View>
            <View style={styles.statBox}>
              <Text style={[styles.statNum, { color: theme.colors.text }]}>{stats.products}</Text>
              <Text style={[styles.statName, { color: theme.colors.textTertiary }]}>منتج</Text>
            </View>
            <View style={styles.statBox}>
          ...
          {isExpanded && (
            <View style={styles.expanded}>
              <View style={styles.expItem}>
                ...
                <Text style={[styles.expText, { color: theme.colors.textSecondary }]}>المسوقين: {stats.affiliates}</Text>
              </View>
              {ownerProfile?.phone && (
                <View style={styles.expItem}>
                  <Ionicons name="call-outline" size={14} color={theme.colors.textTertiary} />
                  <Text style={[styles.expText, { color: theme.colors.textSecondary }]}>{ownerProfile.phone}</Text>
                </View>
              )}
            </View>
          )}
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]} edges={['bottom']}>
      ...
      <FlatList
            ...
            <View style={styles.controls}>
              ...
              <View style={styles.filterRow}>
                {FILTERS.map(f => (
                  ...
                  >
                    <Text style={[styles.filterText, { color: filter === f.key ? '#FFF' : theme.colors.textSecondary }]}>{f.label}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
        ...
        ListEmptyComponent={<EmptyState title="لا توجد متاجر" message="لم نجد أي متجر يطابق بحثك." />}
      />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  summaryValue: { ...typography.h3, fontSize: 18, fontFamily: 'Tajawal_800ExtraBold' },
  controls: {
  ...
  statNum: { ...typography.small, fontFamily: 'Tajawal_800ExtraBold', fontSize: 13 },
