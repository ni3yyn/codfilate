...
export default function AdminAssignWilayaScreen() {
  ...
  const load = useCallback(async () => {
    ...
    let arr = [];
    if (Array.isArray(aw)) arr = aw.map((x) => (typeof x === 'string' ? parseInt(x, 10) : x)).filter(Boolean);
    else if (typeof aw === 'string') {
      try {
        const p = JSON.parse(aw);
        arr = Array.isArray(p) ? p.map((x) => (typeof x === 'string' ? parseInt(x, 10) : x)).filter(Boolean) : [];
      } catch (e) { /* ignore */ }
    }
  ...
  if (!profileId) {
    return (
      <SafeAreaView style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }} edges={['top']}>
        <Text style={{ color: theme.colors.textSecondary }}>لم يُحدد مستخدم</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }} edges={['bottom']}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.md, paddingBottom: 100 }}>
        <Card style={{ padding: spacing.md, marginBottom: spacing.md }}>
          <Text style={{ color: theme.colors.textSecondary }}>الدور الحالي: {profile.role}</Text>
          <Button title="حفظ التعيين" onPress={save} loading={saving} variant="gradient" style={{ marginTop: spacing.md }} />
        </Card>
        {wilayas.map((w) => (
          <TouchableOpacity key={w.id} onPress={() => toggle(w.id)} activeOpacity={0.8}>
            <View style={[styles.row, { borderColor: theme.colors.border, backgroundColor: selected.has(w.id) ? theme.primary + '18' : 'transparent' }]}>
