...
const STATUS_MAP = {
  ...
  paid: { label: 'تم الدفع', variant: 'success', icon: 'checkmark-circle-outline' },
...
export default function AdminPayoutsScreen() {
  ...
  const { updatePayoutStatus, fetchPayoutRequests, payoutRequests, isLoading } = useAffiliateStore();
  ...
  const runUpdate = async (id, status) => {
    ...
    if (result.success) {
      showAlert({ title: 'تم', message: 'تم تحديث حالة الطلب', type: 'success' });
      setExpandedId(null);
      setExternalRef('');
      setProofUrl('');
      load();
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.colors.background }]} edges={['bottom']}>
      <UniversalHeader title="إدارة سحوبات الأموال" subtitle="مراجعة ومعالجة طلبات السحب من لوحة التحكم" />
              ...
              return (
                <View key={item.id} style={{ width: cardWidth, marginBottom: spacing.md }}>
                  <Card 
                    ...
                    borderVariant={item.status === 'pending' ? 'thick' : 'default'}
                    accentColor={item.status === 'pending' ? theme.primary : null}
                  >
                    {/* Header Info */}
                    <View style={styles.cardInfo}>
                      ...
                      <View style={styles.requesterSection}>
                        <Avatar name={item.requester?.full_name || '?'} size={32} />
                        <View style={{ flex: 1, marginStart: 8 }}>
                          <Text style={[styles.userName, { color: theme.colors.text }]} numberOfLines={1}>
                            {item.requester?.full_name || 'مجهول'}
                          </Text>
                          <Text style={[styles.userRole, { color: theme.colors.textTertiary }]}>
                            {item.requester?.role === 'merchant' ? 'مورد' : 'مسوق'}
                          </Text>
                        </View>
                      </View>

                      <View style={styles.methodBox}>
                        <Text style={[styles.methodLabel, { color: theme.colors.textTertiary }]}>العملية:</Text>
                        <Text style={[styles.methodValue, { color: theme.colors.textSecondary }]}>
                        ...
                        <View style={styles.expandedContent}>
                          ...
                          <View style={styles.detailRow}>
                            ...
                            <TouchableOpacity onPress={() => copyToClipboard(item.payment_details, 'بيانات الدفع')}>
                              <Text style={[styles.detailValue, { color: theme.primary, fontFamily: 'Tajawal_700Bold' }]}>
...
const styles = StyleSheet.create({
  ...
  methodValue: { ...typography.caption, fontSize: 11, fontFamily: 'Tajawal_500Medium' },
