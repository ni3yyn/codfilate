...
export default function CompletedDeliveries() {
  ...
  const { completedDeliveries, fetchCompletedDeliveries, isLoading } =
    useDeliveryStore();
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <Card
        style={styles.card}
        accentColor={isDelivered ? theme.primary : theme.error}
        accentPosition="left"
      >
        <View style={styles.cardHeader}>
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  (isDelivered ? theme.primary : theme.error) + "10",
              },
            ]}
          >
            <Ionicons
        ...
        {item.failed_reason && (
          <View
            style={[styles.failBox, { backgroundColor: theme.error + "05" }]}
          >
  ...
  const successfulCount = completedDeliveries.filter(
    (d) => d.status === "delivered",
  ).length;
  const failedCount = completedDeliveries.filter(
    (d) => d.status === "failed",
  ).length;
