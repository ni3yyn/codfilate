...
export default function MerchantPayouts() {
  ...
  const handleRequest = async () => {
    ...
    if (!profile.store_id) {
      showAlert({ title: 'خطأ', message: 'لم يتم ربط حسابك بمتجر. يرجى التواصل مع الإدارة.', type: 'error' });
      return;
    }
    ...
    if (result.success) {
      showAlert({ title: 'نجاح', message: 'تم إرسال طلب السحب للمدير الإقليمي بنجاح', type: 'success' });
      setAmount('');
      setDetails('');
      setShowForm(false);
    ...
    >
      <View style={styles.payoutRow}>
        <View style={styles.payoutInfo}>
          ...
          <Text style={[styles.payoutMethod, { color: theme.colors.textSecondary }]}>
      ...
      {(item.admin_notes || item.external_ref || item.payout_proof_url) && (
        <View style={[styles.notesBox, { borderTopColor: theme.colors.divider }]}>
          ...
          {item.external_ref && (
            <Text style={[styles.notesText, { color: theme.primary, marginTop: 4, fontFamily: 'Tajawal_500Medium' }]}>
              🧾 مرجع الدفع: {item.external_ref}
            </Text>
          )}
          {item.payout_proof_url && (
            <TouchableOpacity onPress={() => Linking.openURL(item.payout_proof_url)} style={{ marginTop: 8, flexDirection: 'row', alignItems: 'center', backgroundColor: theme.primary + '15', alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8 }}>
...
const styles = StyleSheet.create({
  ...
  helperText: { ...typography.small, fontFamily: 'Tajawal_500Medium', flex: 1, marginStart: 8 },
  balanceCard: {
    alignItems: 'center',
    marginBottom: spacing.lg,
  },
  balanceLabel: { ...typography.body, color: 'rgba(255,255,255,0.75)', marginBottom: 4 },
